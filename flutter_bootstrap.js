// Bootstrap personnalise de Flutter Web.
//
// On N'UTILISE PAS le service worker de cache de Flutter (il servait d'anciennes
// versions sur iOS). A la place, app-sw.js cache le shell en NETWORK-FIRST : en
// ligne on prend toujours la derniere version (jamais bloque), hors-ligne on
// ressert la copie en cache pour que l'app se lance quand meme.
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.getRegistrations().then(function (registrations) {
    for (var i = 0; i < registrations.length; i++) {
      ...
      // Retirer UNIQUEMENT l'ancien service worker de cache de Flutter, surtout
      // pas push-sw.js (notifications) ni app-sw.js (shell hors-ligne).
  ...
  // Cache du shell de l'app : permet de la lancer sans reseau.
